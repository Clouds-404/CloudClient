# Cloud Launcher Nix Packaging

## Installing a stable release (nixpkgs)

Cloud Launcher is packaged in [nixpkgs](https://github.com/NixOS/nixpkgs/) since 22.11.

Check the [NixOS Wiki](https://wiki.nixos.org/wiki/Cloud_Launcher) for up-to-date instructions.
...
{
  nix.settings = {
    trusted-substituters = [ "https://Cloudlauncher.cachix.org" ];

    trusted-public-keys = [
      "Cloudlauncher.cachix.org-1:9/n/FGyABA2jLUVfY+DEp4hKds/rwO+SCOtbOkDzd+c="
    ];
  };
}
```
...
After adding `github:Cloudlauncher/Cloudlauncher` to your flake inputs, you can access the flake's `packages` output.
...
```nix
{
  inputs = {
    ...
    Cloudlauncher = {
      url = "github:Cloudlauncher/Cloudlauncher";

      # Optional: Override the nixpkgs input of Cloudlauncher to use the same revision as the rest of your flake
      ...
      nixosConfigurations.foo = nixpkgs.lib.nixosSystem {
        modules = [
          ...
          (
            { pkgs, ... }:
            {
              environment.systemPackages = [ Cloudlauncher.packages.${pkgs.system}.Cloudlauncher ];
            }
          )
        ];
      };
...
This will ensure Cloud is built with your system's packages.
      ...
      nixosConfigurations.foo = nixpkgs.lib.nixosSystem {
        modules = [
          ...
          (
            { pkgs, ... }:
            {
              nixpkgs.overlays = [ Cloudlauncher.overlays.default ];

              environment.systemPackages = [ pkgs.Cloudlauncher ];
            }
          )
        ];
      };
...
```shell
nix run github:Cloudlauncher/Cloudlauncher

nix shell github:Cloudlauncher/Cloudlauncher

nix profile install github:Cloudlauncher/Cloudlauncher
```
...
{ pkgs, ... }:
{
  environment.systemPackages = [
    (import (
      builtins.fetchTarball "https://github.com/Cloudlauncher/Cloudlauncher/archive/develop.tar.gz"
    )).packages.${pkgs.system}.Cloudlauncher
  ];
}
```
...
This results in Cloud using your system's libraries
...
```shell
nix-channel --add https://github.com/Cloudlauncher/Cloudlauncher/archive/develop.tar.gz Cloudlauncher

nix-channel --update Cloudlauncher

nix-env -iA Cloudlauncher.Cloudlauncher
```
...
- `Cloudlauncher` - The preferred build, wrapped with everything necessary to run the launcher and Minecraft
- `Cloudlauncher-unwrapped` - A minimal build that allows for advanced customization of the launcher's runtime environment

### Customizing wrapped packages

The wrapped package (`Cloudlauncher`) offers some build parameters to further customize the launcher's environment.
...
- `jdks` (default: `[ jdk21 jdk17 jdk8 ]`) Java runtimes added to `CloudClient_JAVA_PATHS` variable
