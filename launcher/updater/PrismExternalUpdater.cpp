...
#include "CloudExternalUpdater.h"
...
class CloudExternalUpdater::Private {
   public:
    QDir appDir;
    QDir dataDir;
    QTimer updateTimer;
...
CloudExternalUpdater::CloudExternalUpdater(QWidget* parent, const QString& appDir, const QString& dataDir)
    : priv(new CloudExternalUpdater::Private())
{
    priv->appDir = QDir(appDir);
    priv->dataDir = QDir(dataDir);
    auto settingsFile = priv->dataDir.absoluteFilePath("Cloudlauncher_update.cfg");
...
CloudExternalUpdater::~CloudExternalUpdater()
{
...
void CloudExternalUpdater::checkForUpdates()
{
    checkForUpdates(true);
}

void CloudExternalUpdater::checkForUpdates(bool triggeredByUser) const
{
...
bool CloudExternalUpdater::getAutomaticallyChecksForUpdates()
{
    return priv->autoCheck;
}

double CloudExternalUpdater::getUpdateCheckInterval()
{
    return priv->updateInterval;
}

bool CloudExternalUpdater::getBetaAllowed()
{
    return priv->allowBeta;
}

void CloudExternalUpdater::setAutomaticallyChecksForUpdates(bool check)
{
...
void CloudExternalUpdater::setUpdateCheckInterval(double seconds)
{
...
void CloudExternalUpdater::setBetaAllowed(bool allowed)
{
...
void CloudExternalUpdater::resetAutoCheckTimer() const
{
...
void CloudExternalUpdater::connectTimer()
{
    connect(&priv->updateTimer, &QTimer::timeout, this, &CloudExternalUpdater::autoCheckTimerFired);
}

void CloudExternalUpdater::disconnectTimer()
{
    disconnect(&priv->updateTimer, &QTimer::timeout, this, &CloudExternalUpdater::autoCheckTimerFired);
}

void CloudExternalUpdater::autoCheckTimerFired() const
{
    qDebug() << "Auto update Timer fired";
    checkForUpdates(false);
}

void CloudExternalUpdater::offerUpdate(const QString& versionName,
...
void CloudExternalUpdater::performUpdate(const QString& versionTag) const
{
    QProcess proc;
    auto exeName = QStringLiteral("%1_updater").arg(BuildConfig.LAUNCHER_APP_BINARY_NAME);
#ifdef Q_OS_WIN32
