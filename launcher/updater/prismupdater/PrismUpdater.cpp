...
/*
 *  Cloud Launcher - Minecraft Launcher
...
#include "updater/Cloudupdater/UpdaterDialogs.h"
...
void appDebugOutput(QtMsgType type, const QMessageLogContext& context, const QString& msg)
{
    ...
    CloudUpdaterApp* app = static_cast<CloudUpdaterApp*>(QCoreApplication::instance());
...
CloudUpdaterApp::CloudUpdaterApp(int& argc, char** argv) : QApplication(argc, argv)
{
    ...
    parser.setApplicationDescription(QObject::tr("An auto-updater for Cloud Launcher"));
    ...
    if (!dirParam.isEmpty()) {
        // the dir param. it makes Cloud launcher data path point to whatever the user specified
        // on command line
    ...
    m_updateLogPath = FS::PathCombine(m_dataPath, "logs", "Cloud_launcher_update.log");
    ...
    QString Cloud_executable = FS::PathCombine(binPath, BuildConfig.LAUNCHER_APP_BINARY_NAME);
#if defined Q_OS_WIN32
    Cloud_executable.append(".exe");
#endif

    if (!QFileInfo(Cloud_executable).isFile()) {
        showFatalErrorMessage(tr("Unsupported Installation"), tr("The updater can not find the main executable."));
    }

    m_CloudExecutable = Cloud_executable;

    auto Cloud_update_url = parser.value("update-url");
    if (Cloud_update_url.isEmpty())
        Cloud_update_url = BuildConfig.UPDATER_GITHUB_REPO;

    m_CloudRepoUrl = QUrl::fromUserInput(Cloud_update_url);
    ...
    auto version = parser.value("Cloud-version");
    if (!version.isEmpty()) {
        if (version.contains('-')) {
        ...
        m_CloudVersionMajor = version_parts.takeFirst().toInt();
        m_CloudVersionMinor = version_parts.takeFirst().toInt();
        if (!version_parts.isEmpty())
            m_CloudVersionPatch = version_parts.takeFirst().toInt();
        else
            m_CloudVersionPatch = 0;
    }

    m_allowPreRelease = parser.isSet("pre-release");

    auto marker_file_path = QDir(m_rootPath).absoluteFilePath(".Cloud_launcher_updater_unpack.marker");
    ...
    if (marker_file.exists()) {
        ...
        QMetaObject::invokeMethod(this, &CloudUpdaterApp::loadReleaseList, Qt::QueuedConnection);
    }
}

CloudUpdaterApp::~CloudUpdaterApp()
{
...
void CloudUpdaterApp::fail(const QString& reason)
{
...
void CloudUpdaterApp::abort(const QString& reason)
{
    qCritical() << qPrintable(reason);
    m_status = Aborted;
    exit(2);
...
void CloudUpdaterApp::showFatalErrorMessage(const QString& title, const QString& content)
{
    m_status = Failed;
...
void CloudUpdaterApp::run()
{
    qDebug() << "found" << m_releases.length() << "releases on github";
    qDebug() << "loading exe at" << m_CloudExecutable;
    ...
    if (!loadCloudVersionFromExe(m_CloudExecutable)) {
        m_CloudVersion = BuildConfig.printableVersionString();
        m_CloudVersionMajor = BuildConfig.VERSION_MAJOR;
        m_CloudVersionMinor = BuildConfig.VERSION_MINOR;
        m_CloudVersionPatch = BuildConfig.VERSION_PATCH;
        m_prsimVersionChannel = BuildConfig.VERSION_CHANNEL;
        m_CloudGitCommit = BuildConfig.GIT_COMMIT;
    }
    m_status = Succeeded;

    qDebug() << "Executable reports as:" << m_CloudBinaryName << "version:" << m_CloudVersion;
    qDebug() << "Version major:" << m_CloudVersionMajor;
    qDebug() << "Version minor:" << m_CloudVersionMinor;
    qDebug() << "Version minor:" << m_CloudVersionPatch;
    qDebug() << "Version channel:" << m_prsimVersionChannel;
    qDebug() << "Git Commit:" << m_CloudGitCommit;
    ...
    if (m_isFlatpak) {
        showFatalErrorMessage(tr("Updating flatpack not supported"), tr("Actions outside of checking if an update is available are not "
                                                                        "supported when running the flatpak version of Cloud Launcher."));
        return;
    }
    if (m_isAppimage) {
        bool result = true;
...
void CloudUpdaterApp::moveAndFinishUpdate(QDir target)
{
    ...
    if (error) {
        ...
        auto fail_marker = FS::PathCombine(m_dataPath, ".Cloud_launcher_update.fail");
        FS::copy(m_updateLogPath, fail_marker).overwrite(true)();
    } else {
        logUpdate(tr("Update succeed."));
        auto success_marker = FS::PathCombine(m_dataPath, ".Cloud_launcher_update.success");
        FS::copy(m_updateLogPath, success_marker).overwrite(true)();
    }
    auto update_lock_path = FS::PathCombine(m_dataPath, ".Cloud_launcher_update.lock");
...
void CloudUpdaterApp::printReleases()
{
...
QList<GitHubRelease> CloudUpdaterApp::nonDraftReleases()
{
...
QList<GitHubRelease> CloudUpdaterApp::newerReleases()
{
    QList<GitHubRelease> newer;
    for (auto rls : nonDraftReleases()) {
        if (rls.version > m_CloudVersion)
            newer.append(rls);
    }
    return newer;
}

GitHubRelease CloudUpdaterApp::selectRelease()
{
    ...
    SelectReleaseDialog dlg(Version(m_CloudVersion), releases);
...
QList<GitHubReleaseAsset> CloudUpdaterApp::validReleaseArtifacts(const GitHubRelease& release)
{
...
GitHubReleaseAsset CloudUpdaterApp::selectAsset(const QList<GitHubReleaseAsset>& assets)
{
...
void CloudUpdaterApp::performUpdate(const GitHubRelease& release)
{
...
QFileInfo CloudUpdaterApp::downloadAsset(const GitHubReleaseAsset& asset)
{
...
bool CloudUpdaterApp::callAppImageUpdate()
{
...
void CloudUpdaterApp::clearUpdateLog()
{
    FS::deletePath(m_updateLogPath);
}

void CloudUpdaterApp::logUpdate(const QString& msg)
{
...
void CloudUpdaterApp::performInstall(QFileInfo file)
{
    ...
    if (update_lock.exists()) {
        ...
        auto infoMsg =
            tr("This installation has a update lock file present at: %1\n"
               ...
               "proceeding.\n"
               "Check the Cloud Launcher updater log at: \n"
               "%7\n"
    ...
    auto changelog_path = FS::PathCombine(m_dataPath, ".Cloud_launcher_update.changelog");
    FS::write(changelog_path, m_install_release.body.toUtf8());

    logUpdate(tr("Updating from %1 to %2").arg(m_CloudVersion).arg(m_install_release.tag_name));
    if (m_isPortable || file.fileName().endsWith(".zip") || file.fileName().endsWith(".tar.gz")) {
        write_lock_file(update_lock_path, QDateTime::currentDateTime(), m_CloudVersion, m_install_release.tag_name, m_rootPath, m_dataPath);
...
void CloudUpdaterApp::unpackAndInstall(QFileInfo archive)
{
    logUpdate(tr("Backing up install"));
    backupAppDir();

    if (auto loc = unpackArchive(archive)) {
        auto marker_file_path = loc.value().absoluteFilePath(".Cloud_launcher_updater_unpack.marker");
...
void CloudUpdaterApp::backupAppDir()
{
    auto manifest_path = FS::PathCombine(m_rootPath, "manifest.txt");
    QFileInfo manifest(manifest_path);
    ...
    if (file_list.isEmpty()) {
        ...
        if (BuildConfig.BUILD_ARTIFACT.toLower().contains("linux")) {
            file_list.append({ "cloudclientlauncher", "bin", "share", "lib" });
        } else {  // windows by process of elimination
            file_list.append({
                "jars",
                "cloudclientlauncher.exe",
                "cloudclientlauncher_filelink.exe",
                "cloudclientlauncher_updater.exe",
                "qtlogging.ini",
                "imageformats",
                "iconengines",
                "platforms",
    ...
    auto backup_dir =
        FS::PathCombine(app_dir.absolutePath(),
                        QStringLiteral("backup_") + QString(m_CloudVersion).replace(s_replaceRegex, QString("_")) + "-" + m_CloudGitCommit);
    FS::ensureFolderPathExists(backup_dir);
    auto backup_marker_path = FS::PathCombine(m_dataPath, ".Cloud_launcher_update_backup_path.txt");
...
std::optional<QDir> CloudUpdaterApp::unpackArchive(QFileInfo archive)
{
    auto temp_extract_path = FS::PathCombine(m_dataPath, "Cloud_launcher_update_release");
...
bool CloudUpdaterApp::loadCloudVersionFromExe(const QString& exe_path)
{
    QProcess proc = QProcess();
    ...
    if (first_parts.length() < 2)
        return false;
    m_CloudBinaryName = first_parts.takeFirst();
    auto version = first_parts.takeFirst().trimmed();
    m_CloudVersion = version;
    if (version.contains('-')) {
        auto index = version.indexOf('-');
        m_prsimVersionChannel = version.mid(index + 1);
        version = version.left(index);
    ...
    if (!version_parts.isEmpty())
        ...
        m_CloudVersionPatch = 0;
    m_CloudGitCommit = lines.takeFirst().simplified();
    return true;
}

void CloudUpdaterApp::loadReleaseList()
{
    auto github_repo = m_CloudRepoUrl;
...
void CloudUpdaterApp::downloadReleasePage(const QString& api_url, int page)
{
    int per_page = 30;
    ...
    connect(download.get(), &Net::Download::failed, this, &CloudUpdaterApp::downloadError);
...
int CloudUpdaterApp::parseReleasePage(const QByteArray* response)
{
...
GitHubRelease CloudUpdaterApp::getLatestRelease()
{
...
bool CloudUpdaterApp::needUpdate(const GitHubRelease& release)
{
    auto current_ver = Version(QString("%1.%2.%3").arg(m_CloudVersionMajor).arg(m_CloudVersionMinor).arg(m_CloudVersionPatch));
    return current_ver < release.version;
}

void CloudUpdaterApp::downloadError(QString reason)
{
