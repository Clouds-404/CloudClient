...
#endif
#else
#include "updater/CloudExternalUpdater.h"
#endif
...
Application::Application(int& argc, char** argv) : QApplication(argc, argv)
{
    ...
    /*
     * Establish the mechanism for communication with an already running Cloudlauncher that uses the same data path.
...

    {
        ...
        if (!migrated)
            migrated = handleDataMigration(
                dataPath, FS::PathCombine(QStandardPaths::writableLocation(QStandardPaths::AppDataLocation), "../../Cloudlauncher"),
                "Cloud Launcher", "Cloudlauncher.cfg");
    }
    ...
    // Initialize application settings
    {
        ...
        m_settings.reset(
            new INISettingsObject({ BuildConfig.LAUNCHER_CONFIGFILE, "Cloudlauncher.cfg", "polymc.cfg", "multimc.cfg" }, this));
        ...
        m_settings->registerSetting("CloudAutoServers", true);
    ...
    m_CloudNetworkCheck = std::make_unique<CloudNetworkCheck>(m_network.get());
    connect(m_CloudNetworkCheck.get(), &CloudNetworkCheck::shouldReloadNews, this, &Application::shouldReloadNews);
    ...
    // check update locks
    {
        auto update_log_path = FS::PathCombine(m_dataPath, "logs", "Cloud_launcher_update.log");

        auto update_lock = QFileInfo(FS::PathCombine(m_dataPath, ".Cloud_launcher_update.lock"));
        ...
        auto update_fail_marker = QFileInfo(FS::PathCombine(m_dataPath, ".Cloud_launcher_update.fail"));
        ...
        auto update_success_marker = QFileInfo(FS::PathCombine(m_dataPath, ".Cloud_launcher_update.success"));
...
#endif
#else
        m_updater.reset(new CloudExternalUpdater(m_mainWindow, m_rootPath, m_dataPath));
#endif
