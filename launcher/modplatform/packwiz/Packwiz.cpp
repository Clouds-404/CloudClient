...
void V1::updateModIndex(const QDir& index_dir, Mod& mod)
{
    ...
    QTextStream in_stream(&index_file);
    {
        auto tbl = toml::table{ { "name", mod.name.toStdString() },
                                ...
                                { "x-Cloudlauncher-loaders", loaders },
                                { "x-Cloudlauncher-mc-versions", mcVersions },
                                { "x-Cloudlauncher-release-type", mod.releaseType.toString().toStdString() },
                                { "x-Cloudlauncher-version-number", mod.version_number.toStdString() },
                                { "x-Cloudlauncher-dependencies", deps },
                                { "download",
                                  toml::table{
...
auto V1::getIndexForMod(const QDir& index_dir, QString slug) -> Mod
{
    ...
    {  // Basic info
        ...
        mod.releaseType = ModPlatform::IndexedVersionType::fromString(table["x-Cloudlauncher-release-type"].value_or(""));
        if (auto loaders = table["x-Cloudlauncher-loaders"]; loaders && loaders.is_array()) {
        ...
        if (auto versions = table["x-Cloudlauncher-mc-versions"]; versions && versions.is_array()) {
    ...
    mod.version_number = table["x-Cloudlauncher-version-number"].value_or("");
    ...
    {  // dependencies
        auto deps = table["x-Cloudlauncher-dependencies"].as_array();
        if (deps) {
