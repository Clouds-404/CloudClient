...
APIPage::APIPage(QWidget* parent) : QWidget(parent), ui(new Ui::APIPage)
{
    ...
    connect(ui->setHostedServers, &QPushButton::clicked, this, [this] {
        constexpr auto key = CloudNetworkCheck::Result::UsePrimary;
        ui->metaURL->setText(CloudNetworkCheck::metaUrls().value(key));
        ui->legacyFMLLibsURL->setText(CloudNetworkCheck::fmlLibsUrls().value(key));
    });
    connect(ui->setGitHubNew, &QPushButton::clicked, this, [this] {
        constexpr auto key = CloudNetworkCheck::Result::UseNewFallback;
        ui->metaURL->setText(CloudNetworkCheck::metaUrls().value(key));
        ui->legacyFMLLibsURL->setText(CloudNetworkCheck::fmlLibsUrls().value(key));
    });
    connect(ui->setGitHubOld, &QPushButton::clicked, this, [this] {
        constexpr auto key = CloudNetworkCheck::Result::UseOldFallback;
        ui->metaURL->setText(CloudNetworkCheck::metaUrls().value(key));
        ui->legacyFMLLibsURL->setText(CloudNetworkCheck::fmlLibsUrls().value(key));
    });
...
void APIPage::loadSettings()
{
    ...
    ui->autoServersCheckBox->setChecked(s->get("CloudAutoServers").toBool());
}
...
void APIPage::applySettings()
{
    ...
    s->set("CloudAutoServers", ui->autoServersCheckBox->isChecked());
}
