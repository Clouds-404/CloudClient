    ...
    for (auto& url : urls) {
        ...
        if (!url.isLocalFile()) {  // download the remote resource and identify
            ...
            if (url.scheme() == "curseforge" || (url.scheme() == BuildConfig.LAUNCHER_APP_BINARY_NAME && url.host() == "install")) {
                ...
                continue;
            } else if ((url.scheme() == "Cloudlauncher" || url.scheme() == BuildConfig.LAUNCHER_APP_BINARY_NAME)
                        && isExternalURLImport) {
                // Cloudlauncher URL protocol modpack import
                // works for any Cloud fork
                // preferred import format: Cloudlauncher://import?url=ENCODED
                ...
                // alternative import format: Cloudlauncher://import/ENCODED
...
void MainWindow::globalSettingsClosed()
{
    ...
    // but Cloud Launcher exits abnormally, causing the window state to never be saved:
